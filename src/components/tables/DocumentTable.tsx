...
export function DocumentTable() {
	...
	const [showRecentValuesOnly, setShowRecentValuesOnly] = useAtom(
		documentRecentSettingAtom,
	);
	...
	const filteredValues = useMemo(() => {
		...
		for (const [fieldName, caseValues] of Object.entries(values)) {
			...
			if (showRecentValuesOnly) {
				...
				recentValues = Object.entries(grouped)
					.slice(0, 3)
					.flatMap(([_, values]) => values) as Array<CaseValue>;
			}
			const fieldDisplayName =
				caseFieldMap[fieldName]?.displayName.toLowerCase() ?? "";
			if (fieldDisplayName.includes(lowerSearch)) {
				...
				for (const caseValue of recentValues) {
					const documents = caseValue.documents.filter((doc) =>
	...
	const caseFields = selectedDocCase
		? selectedDocCase.caseFields
		: data.flatMap((_case) => _case.caseFields);
	const noValuesAvailableText = caseFields.every(
		(field) => (filteredValues[field.name]?.length ?? 0) === 0,
	) ? (
	...
	return (
		<>
			...
			<Stack direction="row" flexWrap="wrap" spacing={2} alignItems="start">
				<CategoryFilter
					selectedDocCase={selectedDocCase}
					setSelectedDocCase={setSelectedDocCase}
				/>
				<RecentFilter
					recentValues={showRecentValuesOnly}
					setRecentValues={setShowRecentValuesOnly}
				/>
			</Stack>
		...
		<>
			<Button startIcon={icon} onClick={handleClick} disableRipple>
				<Typography variant="button" lineHeight={1}>
					{recentValues ? onlyRecentText : allText}
				</Typography>
			</Button>
			<Popover
				id={id}
...
function CategoryFilter({ selectedDocCase, setSelectedDocCase }) {
	...
	const availableDocCases = useMemo(
		() =>
			data.filter(
				(_case) =>
					_case.caseFields
						.map((f) => values[f.name].length)
						.reduce((a, b) => a + b) > 0,
			),
		[data, values],
	);
			...
			{availableDocCases.map((docCase) => {
				...
				return (
					<Chip
						...
						size="small"
						onClick={() => {
							setSelectedDocCase(isSelected ? undefined : docCase);
						}}
					/>
				);
			})}
		...
		<Stack spacing={0.5}>
			...
			<Stack>
				{entries.map(([key, values]) => (
					<MonthGroup key={key} month={key} values={values} />
				))}
			</Stack>
		</Stack>
	);
										...
										<Stack direction="row" justifyContent="end" spacing={1}>
											...
											<ResponsiveDialogClose>
												<Button
													variant="contained"
													color="destructive"
													onClick={() => onDelete(caseValue.id, document.id)}
												>
													{t("Delete")}
												</Button>
											</ResponsiveDialogClose>
