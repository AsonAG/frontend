...
function NewEventDialogContent({ close }: { close: () => void }) {
	...
	useEffect(() => {
		...
		async function fetchInitialState() {
			try {
				const employeePromise = getPayrollEmployees(params)
					.withActive()
					.withSignal(controller.signal)
					.fetchJson();
				if (params.employeeId) {
					const casesPromise = getEmployeeCases(
						params,
						"NewEvent",
						controller.signal,
					);
					const [employees, cases]: [Array<Employee>, Array<any>] =
						await Promise.all([employeePromise, casesPromise]);
	...
	useEffect(() => {
		...
		if (state.page === "employee" && state.cases === null) {
			...
			const getCases = () =>
				getEmployeeCases(
					{ ...params, employeeId: state.employee.id },
					"NewEvent",
					controller.signal,
				);
			fetchCases(getCases);
		...
		if (state.page === "company" && state.cases === null) {
			const controller = new AbortController();
			const getCases = () =>
				getCompanyCases(params, "NewEvent", controller.signal);
			fetchCases(getCases);
			return () => controller.abort();
		}
	...
	return (
		<ResponsiveDialogContent containerWidth disablePadding spacing={0}>
			<VisuallyHidden>
				...
				<ResponsiveDialogDescription>
					{t("Mask for selecting a new event to report")}
				</ResponsiveDialogDescription>
			</VisuallyHidden>
			<Stack
				direction="row"
			...
			<Command
				...
				filter={(value, search, keywords) => {
					...
					if (allKeywords.toLowerCase().includes(search.toLowerCase()))
						return 0.9;
					return 0;
				}}
			>
				<Command.Input
					value={state.search}
					onValueChange={(s) => dispatch({ type: "set-search", search: s })}
					autoFocus
					placeholder={t(state.placeholder)}
				/>
				<LoadingCommandList state={state}>
					<ContextCommandGroup state={state} dispatch={dispatch} />
					<EmployeeCasesCommandGroup
						state={state}
						dispatch={dispatch}
						close={close}
					/>
					<CompanyCasesCommandGroup
						state={state}
						dispatch={dispatch}
						close={close}
					/>
...
function ContextChip({ state }: { state: State }) {
	...
	const label =
		state.page === "employee"
			? getEmployeeDisplayString(state.employee)
			: t("Company event");
...
function LoadingCommandList({
	...
	return <Command.List>{elements}</Command.List>;
}

function ContextCommandGroup({
	state,
	dispatch,
}: {
	state: State;
	...
	const employees = useMemo(
		() =>
			employeeState?.map((employee: Employee) => (
				<Command.Item
					key={employee.id}
					onSelect={() =>
						dispatch({ type: "select-employee", employee: employee })
					}
				>
					<CommandItem
						text={getEmployeeDisplayString(employee)}
						subtext={employee.identifier}
					/>
				</Command.Item>
			)),
		[employeeState],
	...
	return (
		<>
			<Command.Group heading={t("Company")}>
				<Command.Item onSelect={() => dispatch({ type: "select-company" })}>
					{t("Company event")}
				</Command.Item>
			</Command.Group>
			{employees && (
				<Command.Group heading={t("Employees")}>{employees}</Command.Group>
			)}
		</>
	);
}
...
function EmployeeCasesCommandGroup({
	state,
	close,
	dispatch,
}: {
	...
	const casesState =
		state.initialized && state.page === "employee" ? state.cases : null;
	function onCaseSelected(_case: AvailableCase) {
		const employee =
			state.initialized && state.page === "employee" && state.employee;
		if (!employee) throw new Error("no employee found"); // this should never happen
		const path = generatePath(
			basePath + "/employees/:employeeId/new/:caseName",
			{
				...params,
				employeeId: employee.id,
				caseName: encodeURIComponent(_case.name),
			},
		);
		navigate(path);
		close();
	...
	const cases = useMemo(
		() =>
			casesState?.map((_case) => (
				<Command.Item
					key={_case.id}
					onSelect={() => onCaseSelected(_case)}
					keywords={getCaseKeywords(_case)}
				>
	...
	if (!isEmployeeCasesPage || (casesState?.length ?? 0) === 0) return;

	return <Command.Group>{cases}</Command.Group>;
}

function CompanyCasesCommandGroup({
	state,
	close,
	dispatch,
}: {
	...
	const params = useParams();
	const casesState =
		state.initialized && state.page === "company" ? state.cases : null;
	const basePath = useCaseFormBasePath("/orgs/:orgId/payrolls/:payrollId");
	function onCaseSelected(_case: AvailableCase) {
		const path = generatePath(basePath + "/company/new/:caseName", {
			...params,
			caseName: encodeURIComponent(_case.name),
		});
		navigate(path);
	...
	if (!isCompanyCasesPage || (casesState?.length ?? 0) === 0) return;
...
function reducer(state: State, action: Action): State {
	switch (action.type) {
		...
		case "select-company":
			if (!state.initialized) throw new Error("not initialized");
			if (state.page === "employee") throw new Error("invalid action");
			return {
				...state,
				page: "company",
				search: "",
		...
		case "context":
			if (!state.initialized) throw new Error("not initialized");
			if (state.page === "context") return state;
			return {
				...state,
				page: "context",
				search: "",
