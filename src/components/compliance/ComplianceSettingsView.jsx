...
import {
	checkInteroperabilityCompliance,
	pingCompliance,
...
import {
	EnterpriseCertificatePicker,
	TransmitterCertificatePicker,
} from "./ComplianceCertificatePicker";
...
const productionUrls = [defaultProductionUrl];
...
function ComplianceSettingsView() {
	...
	useUpdateEffect(
		() => dispatch({ settings: loadedSettings, type: "set_settings" }),
		[loadedSettings],
	);

	const onSave = () =>
		submit(settings, { method: "post", encType: "application/json" });
		...
		<Stack spacing={2} flex={1}>
			<Stack spacing={2} direction="row" alignItems="center">
				...
				<Select
					...
					sx={{ flex: 1 }}
					onChange={(e) =>
						dispatch({ value: e.target.value, type: "set_submission_url" })
					}
					displayEmpty
				>
					<ListSubheader>{t("Production")}</ListSubheader>
					{productionUrls.map((url) => (
						<MenuItem
							key={url}
							value={url === defaultProductionUrl ? null : url}
						>
							{url}
						</MenuItem>
					))}
			...
			<Stack spacing={2} direction="row" alignItems="center">
				...
				<Select
					value={settings.monitoringId}
					sx={{ flex: 1 }}
					onChange={(e) =>
						dispatch({ value: e.target.value, type: "set_monitoring_id" })
					}
					displayEmpty
				>
			...
			<Stack spacing={2} direction="row" alignItems="center">
				...
				<TransmitterCertificatePicker
					value={settings.transmitterCertificate}
					setValue={(value) =>
						dispatch({ value, type: "set_transmitter_certificate" })
					}
				/>
			</Stack>
			<Stack spacing={2} direction="row" alignItems="center">
				...
				<EnterpriseCertificatePicker
					value={settings.uidCertificate}
					setValue={(value) =>
						dispatch({ value, type: "set_enterprise_certificate" })
					}
				/>
			</Stack>
			<Button variant="contained" sx={{ alignSelf: "end" }} onClick={onSave}>
				{t("Save")}
			</Button>
		</Stack>
	);
...
function ComplianceResponse({ response, successMessage }) {
	...
	const message = response.errors ? (
		<Alert severity="error" variant="filled">
			<Typography>{response.errors}</Typography>
		</Alert>
	) : (
...
function Ping() {
	...
	return (
		<Stack spacing={2} flex={1}>
			<Button variant="contained" onClick={execute} sx={{ alignSelf: "start" }}>
				{t("Ping")}
			</Button>
			<Divider flexItem />
			<ComplianceResponse
				response={response}
				successMessage={t("Ping successful!")}
			/>
		</Stack>
	);
}
...
function CheckInteroperability() {
	...
	const execute = async () => {
		const response = await checkInteroperabilityCompliance(
			params,
			secondOperand,
		);
		setResponse(response);
	...
	return (
		<Stack spacing={2} flex={1}>
			<Stack direction="row" alignContent="start" spacing={2}>
				...
				>
					{t("Check interoperability")}
				</Button>
			</Stack>
			<Divider flexItem />
			<ComplianceResponse
				response={response}
				successMessage={t("CheckInteroperability successful!")}
			/>
		</Stack>
	);
}
...
function TabPanel(props) {
	...
	return (
		<Box sx={{ px: 4 }} flex={1} display="flex">
			{children}
		</Box>
	);
}
