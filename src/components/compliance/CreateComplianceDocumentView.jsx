...
function UploadComplianceDocumentView() {
  ...
  const onDrop = useCallback(acceptedFiles => {
    const upload = async () => {
      ...
      submit({type: "upload", document}, { method: "post", encType: "application/json" })
    }
    upload();
  }, []);
  ...
  return (
    <Dropzone {...getRootProps({isDragActive})}>
      ...
      <Stack justifyContent="center" height="100%">
        {
          isDragActive ?
            <Typography alignSelf="center">{t("Drop the files here ...")}</Typography> :
            <Typography alignSelf="center">{t("Drag and drop your compliance documents here, or click to select files")}</Typography>
        }
      </Stack>
    </Dropzone>
  )
...
function GenerateDocument() {
  ...
  const [date, setDate] = useState(dayjs().utc().subtract(1, 'year').startOf('year'));
  const submit = useSubmit();
  const navigation = useNavigation();

  const onGenerate = () => {
  const reportRequest = {"language":"German","userId":user.id,"parameters":{"YearAndMonth":date.toISOString(),"AHV":"true","FAK":"true","UVG":"true","UVGZ":"true","KTG":"true","WageStatement":"true"}};
    submit({type: "generate", reportRequest}, {method: "post", encType: "application/json"});
  }
  ...
  return (
    <Stack spacing={1} alignItems="start">
      ...
      <Button variant="contained" onClick={onGenerate} startIcon={icon} disabled={isGenerating}>{t("Generate")}</Button>
    </Stack>
  )
}
