...
export function TransmitterCertificatePicker(props) {
	...
	return (
		<ComplianceCertificatePicker
			nullValueText={t("Default certificate")}
			certificateType="Transmitter"
			{...props}
		/>
	);
}
...
export function EnterpriseCertificatePicker(props) {
	...
	return (
		<ComplianceCertificatePicker
			nullValueText={t("No certificate")}
...
function ComplianceCertificatePicker(props) {
	...
	const [passwordDialogForCertificate, setPasswordDialogForCertificate] =
		useState(null);
	...
	return (
		<>
			...
			<CertificateDialog
				...
				setValue={setValue}
			/>
			{passwordDialogForCertificate && (
				<PasswordDialog setCertificatePassword={setPassword} />
			)}
		</>
	);
}
...
function PasswordDialog({ setCertificatePassword }) {
	...
	return (
		<Dialog open>
			...
			<DialogActions>
				<Button onClick={() => setCertificatePassword(password)}>
					{t("Bestätigen")}
				</Button>
			</DialogActions>
		</Dialog>
...
function CertificateDialog({
	...
	return (
		<Dialog open={open} sx={dialogSx} fullScreen={fullScreen}>
			...
			<DialogContent dividers sx={{ p: 0 }}>
				...
				<Divider />
				<List sx={{ px: 1 }}>
					<ListItemButton selected={!value} onClick={() => setValue(null)}>
						{nullValueText}
					</ListItemButton>
					{fetcher.data ? (
						fetcher.data.map((cert) => (
							<ListItemButton
								key={cert.id}
								selected={cert.id === value?.id}
								onClick={() => setValue(cert)}
							>
								{cert.name}
							</ListItemButton>
						))
