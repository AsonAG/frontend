...
function NewPayrunView() {
	...
	const [state, dispatch] = useReducer(
		reducer,
		{ payrun, employees, parameters: null },
		createInitialState,
	);
	...
	const updateTextValue = (type) => (event) =>
	...
	return (
		<Stack spacing={2}>
			...
			<TextField
				label={t("Name")}
				value={state.jobName}
				onChange={updateTextValue("set_job_name")}
				onBlur={() => dispatch({ type: "ensure_job_name" })}
			/>
			<TextField
				label={t("Reason")}
				value={state.jobReason}
				onChange={updateTextValue("set_job_reason")}
			/>
			<EmployeeSelector
				allEmployees={employees}
				selectedEmployees={state.employees}
				updateEmployees={(e) => dispatch({ type: "set_employees", value: e })}
			/>
			<TextField
				label={t("Forecast")}
				value={state.forecast}
				onChange={updateTextValue("set_forecast")}
			/>
			{errorMessage && (
			...
			<Stack direction="row" alignSelf="end" spacing={2}>
				<Button component={Link} to=".." disabled={isProcessing}>
					{t("Back")}
				</Button>
				<Button
...
const getJobName = (period) =>
	`${i18next.t("Payrun")} ${period.format("MMMM")} ${period.year()}`;
const iconProps = {
	size: "1em",
	sx: {
		color: "common.white",
