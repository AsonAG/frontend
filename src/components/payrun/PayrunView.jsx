		...
		<PaginatedContent>
			<Paper variant="outlined">
				<Stack divider={<Divider flexItem />}>
					{jobs.items.map((job) => (
						<PayrunJobRow key={job.id} payrunJob={job} />
					))}
				</Stack>
			</Paper>
		</PaginatedContent>
...
function DraftPayrun() {
	...
	return drafts.length > 0 ? (
		<DraftPayrunJobRow payrunJob={drafts[0]} />
	) : (
		<NewPayrunJobRow />
	);
}
...
function DraftPayrunJobRow({ payrunJob }) {
	...
	const buttons = (
		<Stack direction="row">
			<Tooltip title={t("Abort")}>
				<IconButton
					sx={{ "&:hover": { color: (theme) => theme.palette.error.light } }}
					size="small"
					onClick={abort}
				>
					<Clear />
			...
			<Tooltip title={t("Release")}>
				<IconButton color="primary" size="small" onClick={complete}>
					<DoneAll />
				</IconButton>
			</Tooltip>
		</Stack>
	...
	return (
		<Paper variant="outlined">
			<Row
				icon={icon}
				title={payrunJob.name}
				subtitle={payrunJob.reason}
				buttons={buttons}
...
function useChangeStatus(payrunJobId) {
	...
	const changeStatus = (newStatus) =>
		submit(
			{ status: newStatus, type: "change_status", jobId: payrunJobId },
			{ method: "post", encType: "application/json" },
		);
	return {
		abort: () => changeStatus("Abort"),
...
function JobIcon({ payrunJob }) {
	...
	return (
		<Tooltip title={t("PayrunJobStatus." + payrunJob.jobStatus)}>
