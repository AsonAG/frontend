...
function PayrollSelector() {
	...
	const handleClick = (event) => {
		const route = matchRoutes(routeData, window.location)
			.map((r) => r.route.path)
			.join("/");
		if (route) {
			...
			const menuItems = payrolls.map((p) => {
				const to =
					"/" + generatePath(route, { ...queryParams, payrollId: p.id });
				return (
					<MenuItem key={p.id} component={Link} to={to} onClick={handleClose}>
						{p.name}
					</MenuItem>
				);
			});
