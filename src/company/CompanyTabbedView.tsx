...
import { Navigate, useLoaderData, useOutlet } from "react-router-dom";
...
export function CompanyTabbedView() {
	...
	const onboardingTab = onboardingTaskCount > 0 && (
		<TabLink
			title={t("Onboarding")}
			to="onboarding"
			badgeCount={onboardingTaskCount}
		/>
	);
	if (!outlet && onboardingTaskCount > 0) {
		return <Navigate to="onboarding" replace />;
	}
	return <EventTabbedView startTabs={onboardingTab} endTabs={<ConfigTabs />} />;
}
	...
	return (
		<>
			<TabLink
				title={t("Wage types")}
				to="wagetypemaster"
				badgeCount={missingWageTypeAccountInfoCount}
				asDot
			/>
