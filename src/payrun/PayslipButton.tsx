  ...
  if (payrunPeriodEntry.state !== "Current") {
    return (
      <Tooltip title={generatingTooltip} placement="left">
        <span>
          <IconButton size="small" loading><FilePresentRoundedIcon /></IconButton>
        </span>
      </Tooltip>
    )
  }
  const doc = payrunPeriodEntry.documents?.find(doc => doc.attributes?.type === "payslip");
  if (!doc)
    return;
  return (
    <Tooltip key={doc.id} title={doc.name} placement="left">
      <span>
        <IconButton size="small" component={Link} to={`${payrunPeriodEntry.id}/doc/${doc.id}`} onClick={stopPropagation}><FilePresentRoundedIcon /></IconButton>
      </span>
    </Tooltip>
  )
}
