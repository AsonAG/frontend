...
const errorStates: Record<number, ErrorInformation> = {
	...
	409: {
		severity: "warning",
		text: "In the meantime, there have been changes that require the documents to be reprocessed. Visit this page again as soon as the period has been processed.",
	},
};
...
function ErrorView() {
	...
	const state = errorStates[error.status] ?? {
		severity: "error",
		text: "There was an error generating the documents. The support has been informed. We will fix the issue as soon as possible.",
	};
	return (
		<Alert severity={state.severity} variant="filled">
			<Typography>{t(state.text)}</Typography>
		</Alert>
	);
}
...
function WageStatementSection() {
	...
	const wageStatements = payrunPeriod.entries
		.map((entry) => {
