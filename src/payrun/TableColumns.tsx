...
function getWageTypeTooltipForPreviousValue(
	t: TFunction<"translation", undefined>,
	wageType: string,
	context: CellContext<EntryRow, number | null>,
	previousValueColumnName: string | undefined = undefined,
) {
	if (
		previousValueColumnName &&
		context.table.getState().columnVisibility[previousValueColumnName]
	)
		return null;
	const previousValue = formatValue(
		context.row.original.previousEntry?.[wageType],
	);
	return `${t("Value from previous period")} ${previousValue ?? "-"}`;
}

function TableStatusDot({ row }: { row: EntryRow }) {
	if (row.isEmployed) return;
...
function createColumns() {
	return [
		columnHelper.group({
			id: "employeeTotal",
			header: ({ state, dispatch, completed }) =>
				!completed && <RowSelectionButton state={state} dispatch={dispatch} />,
			columns: [
				columnHelper.accessor("identifier", {
					...
					cell: (props) => {
						return (
							<Typography noWrap>
								{props.getValue()}
								<>&nbsp;</>
								<TableStatusDot row={props.row.original} />
							</Typography>
						);
					},
					header: ({ t }) => t("Id"),
					...
					meta: {
						flex: 1,
						tooltip: (context) =>
							!context.table.getState().columnVisibility.employee
								? `${context.row.original.lastName} ${context.row.original.firstName}`
								: null,
				...
				columnHelper.accessor((row) => `${row.lastName} ${row.firstName}`, {
					id: "employee",
					cell: (props) => {
						const idColumnVisible =
							props.table.getState().columnVisibility.identifier;
						const statusDot = !idColumnVisible ? (
							<>
								&nbsp;
								<TableStatusDot row={props.row.original} />
							</>
					...
					header: ({ t }) => t("Name"),
					footer: ({ table, t }) =>
						!table.getState().columnVisibility.identifier ? t("Total") : null,
					size: 150,
					meta: {
						flex: 1,
						tooltip: (context) =>
							!context.table.getState().columnVisibility.identifier
		...
		columnHelper.group({
			...
			header: ({ table }) =>
				formatValue(table.getState().periodTotals.employerCost),
			columns: [
				columnHelper.accessor("employerCost", {
					id: "employerCost",
					cell: (props) => (
						<Typography noWrap>{formatValue(props.getValue())}</Typography>
					),
					...
					meta: {
						alignment: "right",
						tooltip: (context, t) =>
							getWageTypeTooltipForPreviousValue(t, "employerCost", context),
						headerTooltip: (t) =>
							t("Total costs from the employer's perspective"),
					},
				}),
			],
		}),
		columnHelper.group({
			...
			header: ({ table }) =>
				formatValue(table.getState().periodTotals.previousGross),
			columns: [
		...
		columnHelper.group({
			...
			header: ({ table }) =>
				formatValue(table.getState().periodTotals.diffGross),
			columns: [
				columnHelper.accessor(
					(row) =>
						formatValue(
							(row.grossWage ?? 0) - (row.previousEntry?.grossWage ?? 0),
						),
					{
						id: "grossDiff",
						...
						meta: {
							alignment: "right",
							headerTooltip: (t) =>
								t("Gross wage previous period minus gross wage open period"),
						},
					},
				),
			],
			...
			columns: [
				columnHelper.accessor("grossWage", {
					cell: (props) => {
						...
						const trendingUp =
							!previousGrossWage || grossWage > previousGrossWage;
					...
					meta: {
						alignment: "right",
						tooltip: (context, t) =>
							getWageTypeTooltipForPreviousValue(
								t,
								"grossWage",
								context,
								"grossPreviousPeriod",
			...
			columns: [
				columnHelper.accessor("netWage", {
					...
					meta: {
						alignment: "right",
						tooltip: (context, t) =>
							getWageTypeTooltipForPreviousValue(t, "netWage", context),
		...
		columnHelper.group({
			id: "offsettingTotal",
			header: ({ table }) =>
				formatValue(table.getState().periodTotals.offsetting),
			columns: [
				columnHelper.accessor("offsetting", {
					...
					meta: {
						alignment: "right",
						tooltip: (context, t) =>
							getWageTypeTooltipForPreviousValue(t, "offsetting", context),
						headerTooltip: (t) =>
							t(
			...
			columns: [
				columnHelper.accessor("retro", {
					...
					meta: {
						alignment: "right",
						tooltip: (context, t) =>
							getWageTypeTooltipForPreviousValue(t, "retro", context),
						headerTooltip: (t) =>
							t(
		...
		columnHelper.group({
			...
			header: ({ table }) =>
				formatValue(
					table.getState().periodTotals.openGarnishmentPayoutPreviousPeriod,
				),
			columns: [
				columnHelper.accessor("openGarnishmentPayoutPreviousPeriod", {
					...
					meta: {
						alignment: "right",
						headerTooltip: (t) =>
							t("Garnishments to be paid from the previous period"),
					},
				}),
			],
		}),
		columnHelper.group({
			...
			header: ({ table }) =>
				formatValue(table.getState().periodTotals.openWagePayoutPreviousPeriod),
			columns: [
				columnHelper.accessor("openWagePayoutPreviousPeriod", {
					...
					meta: {
						alignment: "right",
						headerTooltip: (t) =>
							t("Outstanding amount to be paid from the previous period"),
					},
				}),
			],
		}),
		columnHelper.group({
			...
			columns: [
				columnHelper.accessor(
					(row) => (row.paidOut ?? 0) + (row.paidOutGarnishment ?? 0),
					{
						id: "paidOut",
						cell: (props) => (
							<Typography noWrap>{formatValue(props.getValue())}</Typography>
		...
		columnHelper.group({
			id: "garnishmentTotal",
			header: ({ table }) =>
				formatValue(table.getState().periodTotals.garnishment),
			columns: [
		...
		columnHelper.group({
			...
			columns: [
				columnHelper.accessor("openPayout", {
					id: "openPayout",
					cell: (context) => {
						const { hasDetails, popover, openPopover, closePopover } =
							useOpenAmountDetails(context);
					...
					header: ({ t }) => t("Open"),
					footer: (props) =>
						formatValue(props.table.getState().payoutTotals.open),
					size: 110,
					meta: {
						alignment: "right",
						headerTooltip: (t) =>
							t("Outstanding amount to be paid in the open period"),
					},
					sortingFn: (rowA, rowB) => {
		...
		columnHelper.accessor("amount", {
			...
			footer: (props) =>
				formatValue(props.table.getState().payoutTotals.payingOut),
			size: 110,
			meta: {
				alignment: "right",
			},
		...
		columnHelper.display({
			...
			cell: ({ row, t }) => {
				...
				if (relevantEventCount === 0) return <div></div>;
				return (
