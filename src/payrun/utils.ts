...
export function getRowGridSx(
	columnSizes: Array<ColumnDef>,
	spacing: number = 0,
): SxProps<Theme> {
	if (columnSizes.length === 0) return {};
	...
	return {
		...
		maxWidth: (theme) =>
			`calc(${theme.spacing(spacing * (columnSizes.length - 1))} + ${minWidth}px)`,
		width: (theme) =>
			`calc(${theme.spacing(spacing * (columnSizes.length - 1))} + ${minWidth}px)`,
		minWidth: "100%",
	};
