...
import { Stack, Typography, Box, Button, SxProps, Theme } from "@mui/material";
...
export function PeriodCaseValueDialog() {
	...
	const { payrunPeriod } = useRouteLoaderData("payrunperiod") as {
	...
	const employeeId = useMemo(
		() =>
			payrunPeriod.entries.find(
				(entry) => entry.id === params.payrunPeriodEntryId,
			)?.employeeId,
		[params, payrunPeriod],
	);
		...
		<ResponsiveDialog open onOpenChange={onClose}>
			<ResponsiveDialogContent containerWidth>
				<Typography variant="h6">
					{t("New relevant values in open period")}
				</Typography>
				<Table />
				<Stack direction="row" justifyContent="end" spacing={2}>
					<Button to={`../../../hr/employees/${employeeId}`} component={Link}>
						{t("Go to employee data")}
					</Button>
					<ResponsiveDialogClose>
...
function Row({ sx, children }: { sx?: SxProps<Theme> } & PropsWithChildren) {
	return (
		<Box
			display="grid"
			gridTemplateColumns="75px 1fr 1fr 75px 75px"
