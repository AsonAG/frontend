...
export function ReviewOpenPeriod() {
  ...
  const hasUnresolvedControllingCases = controllingData.companyControllingCases.length > 0 || controllingData.employeeControllingCases.length > 0;
  return (
    <>
      <ContentLayout title={<DashboardHeader />}>
        <PeriodDocuments />
        {
          ...
          <Alert severity="error" color="warning"><Typography>{t("The wage controlling has not been completed yet. The period cannot be closed.")}</Typography></Alert>
        }
          ...
          <ResponsiveDialog>
            <Suspense fallback={<Button variant="contained" color="primary" disabled>{t("Close period")}</Button>}>
              <Await resolve={documents} errorElement={<Button variant="contained" color="primary" disabled>{t("Close period")}</Button>}>
                {() =>
                  <ResponsiveDialogTrigger>
                    <Button variant="contained" color="primary" disabled={hasUnresolvedControllingCases}>{t("Close period")}</Button>
                  </ResponsiveDialogTrigger>
                }
              </Await>
            </Suspense>
