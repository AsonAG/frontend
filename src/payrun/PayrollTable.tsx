...
const isRowSelectionEnabled = (row: EntryRow) =>
	!hasControllingTasks(row) && hasOpenAmount(row);
const hasControllingTasks = (entry: EntryRow) =>
	(entry.controllingTasks?.length ?? 0) > 0;
const hasOpenAmount = (entry: EntryRow) => (entry.openPayout ?? 0) > 0;

export const PayrunTable = memo(function PayrunTable({
	entries,
	completed,
}: PayrunTableProps) {
	...
	const table = useReactTable({
		...
		enableRowSelection: (row) =>
			!completed && isRowSelectionEnabled(row.original),
	...
	const onPayout = async (valueDate: string, accountIban: string) => {
		const payouts = entries
			.filter((e) => state.selected[e.id])
			.map((x) => ({ employeeId: x.employeeId, amount: x.amount || 0 }));
		const payout: Payout = {
			// @ts-ignore
			valueDate,
			payouts,
	...
	const rowContainerSx = getRowGridSx(
				...
				>
					{table.getHeaderGroups().map((headerGroup) =>
						headerGroup.headers.map((header) => {
							if (header.isPlaceholder) return <div key={header.id}></div>;
				...
				{!completed && (
					...
					>
						{table.getFooterGroups().map((footerGroup) => {
							if (footerGroup.depth === 0) return;
							return (
								<Box
									key={footerGroup.id}
									component="div"
									sx={{ py: 1.125, ...rowContainerSx }}
								>
	...
	if (row.getCanSelect()) {
		stackSx["&:hover"] = {
			backgroundColor: (theme: Theme) =>
				theme.palette.selection[row.getIsSelected() ? "light" : "dark"],
			cursor: "pointer",
		};
...
function getPeriodTotals(entries: Array<EntryRow>): PeriodTotals {
	...
	for (let entry of entries) {
		...
		totals.openGarnishmentPayoutPreviousPeriod +=
			entry?.openGarnishmentPayoutPreviousPeriod ?? 0;
		totals.openWagePayoutPreviousPeriod +=
			entry?.openWagePayoutPreviousPeriod ?? 0;
...
function reducer(
	...
	switch (action.type) {
		...
		case "toggle_selected":
			newState = {
				...state,
				selected:
					state.selectedEmployeeCount < state.fullSelectionCount
						? getSelectedState(state.entries)
						: {},
			};
			break;
		case "set_amount":
	...
	newState.selectedEmployeeCount = Object.values(newState.selected).filter(
		Boolean,
	).length;
	return newState;
}
...
function getSelectionCount(state: RowSelectionState) {
