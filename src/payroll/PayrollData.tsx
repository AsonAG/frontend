...
} from "../models/AvailableRegulations";
import {
	PayrollRegulations,
	RegulationName,
} from "../models/PayrollRegulations";
...
export function PayrollData() {
	...
	return (
		<Form ref={formRef}>
			<Stack spacing={2}>
				<TextField
					label={t("Organization unit name")}
					value={payrollName}
				...
				<RegulationSelect
					...
					onChange={(value) =>
						dispatch({ type: "set_country", countryRegulation: value })
					}
				...
				<RegulationSelect
					...
					multiple
					onChange={(values) =>
						dispatch({ type: "set_industries", industries: values })
					}
			...
			renderValue={(selected) => (
				<Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
					{selected.map((value) => (
						<Chip
							key={value}
							label={items.find((r) => r.name === value)?.displayName ?? value}
							size="small"
							sx={{ height: 20 }}
						/>
					))}
...
function reducer(
	state: RegulationSelectionState,
	action: RegulationSelectionAction,
): RegulationSelectionState {
	switch (action.type) {
		case "set_country":
			return {
				...state,
				countryRegulations: state.availableRegulations.find(
					(r) => r.name === action.countryRegulation,
				)!,
				selectedRegulations: {
		...
		case "set_industries":
			if (
				state.payrollRegulations.industries.some(
					(x) => !action.industries.includes(x),
				)
			) {
				return {
					...state,
...
function createInitialState({
	...
	const countryRegulations = availableRegulations.find(
		(r) => r.name === payrollRegulations.countryRegulation,
	) ?? {
		name: "",
		displayName: "",
		industries: [],
