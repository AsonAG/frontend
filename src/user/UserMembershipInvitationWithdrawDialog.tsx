...
export function UserMembershipInvitationWithdrawDialogy() {
  ...
  const { userMembershipInvitations, employeeMap } = useRouteLoaderData("userTable") as UserTableData;
    ...
    <ResponsiveDialog open>
      <ResponsiveDialogContent>
        ...
        <ResponsiveDialogDescription asChild>
          <Typography>{t("Do you want to withdraw the invitation for {{name}} to join the organization?", {name})}</Typography>
        </ResponsiveDialogDescription>
        <Stack direction="row" justifyContent="end" spacing={1}>
          <Button component={Link} to="..">{t("Cancel")}</Button>
          <Button variant="contained" color="destructive" onClick={onRemove}>{t("Withdraw")}</Button>
        </Stack>
      </ResponsiveDialogContent>
    </ResponsiveDialog>
  );
