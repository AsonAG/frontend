...
export function UserMembershipRemoveDialog() {
  ...
  const name = useMemo(() => {
    const userMembership = userMemberships.find(user => user.id === userMembershipId)!;
    ...
    <ResponsiveDialog open>
      <ResponsiveDialogContent>
        ...
        <ResponsiveDialogDescription asChild>
          <Typography>{t("Do you want to remove {{name}} from the organization?", {name})}</Typography>
        </ResponsiveDialogDescription>
        <Stack direction="row" justifyContent="end" spacing={1}>
          <Button component={Link} to="..">{t("Cancel")}</Button>
          <Button variant="contained" color="destructive" onClick={onRemove}>{t("Remove")}</Button>
        </Stack>
      </ResponsiveDialogContent>
    </ResponsiveDialog>
  );
