...
): [RoleSelectionState, Dispatch<RoleSelectionReducerAction>] {
	return useReducer(
		reducer,
...
function RoleOption({ active, onSelect, children }) {
	return (
		<Box
			sx={active ? activeRoleOptionSx : inactiveRoleOptionSx}
			onClick={onSelect}
		>
			{children}
		</Box>
...
function AdminRoleOption({
	...
	return (
		<RoleOption
			active={active}
			onSelect={() =>
				!active && dispatch({ type: "set_selected_role", value: "Admin" })
			}
		>
			<Typography>{t("Administrator")}</Typography>
			<Typography variant="body2">
				{t("Grants access to the whole organization")}
			</Typography>
		</RoleOption>
	);
}
...
function ManagerRoleOption(props: RoleSelectionProps) {
	...
	return (
		<RoleOption
			...
			onSelect={() =>
				!active &&
				props.dispatch({ type: "set_selected_role", value: "PayrollManager" })
			}
		>
			<Typography>{t("Manager")}</Typography>
			<Typography variant="body2">
				{t("Grants access to specific organization units")}
			</Typography>
...
function ManagerPayrollSection({
	state,
	dispatch,
	payrolls,
}: RoleSelectionProps) {
	const selectedPayrollIds =
		state.role?.$type === "PayrollManager" ? state.role.payrollIds : [];
	return (
		<Stack>
...
function SelfServiceRoleOption(props: RoleSelectionProps) {
	...
	return (
		<RoleOption
			...
			onSelect={() =>
				!active &&
				props.dispatch({ type: "set_selected_role", value: "SelfService" })
			}
		>
			<Typography>{t("Self Service")}</Typography>
			<Typography variant="body2">
				{t("Grants restricted access to the employee")}
			</Typography>
		</RoleOption>
	);
}
...
function reducer(
	state: RoleSelectionState,
	action: RoleSelectionReducerAction,
): RoleSelectionState {
	switch (action.type) {
		case "set_selected_role":
			if (state.selectedRole === action.value) return state;
			const newSelectedRole: UserRole | null =
				action.value === "PayrollManager" ? null : { $type: action.value };
			return {
		...
		case "remove_manager_payroll":
			...
			const payrollIdsRemoved = state.role.payrollIds.filter(
				(p) => p !== action.value,
			);
			const managerRoleRemoved: UserRole | null =
				payrollIdsRemoved.length > 0
					? { $type: "PayrollManager", payrollIds: payrollIdsRemoved }
					: null;
			return {
				...state,
		...
		case "add_manager_payroll":
			...
			const managerRoleAdded: UserRole = {
				$type: "PayrollManager",
				payrollIds: payrollIdsAdded,
			};
			return {
				...state,
