...
import { ResponsiveDialog, ResponsiveDialogContent, ResponsiveDialogTitle } from "../components/ResponsiveDialog";
...
export function UserMembershipEditDialog() {
  ...
  const { userMemberships, payrolls, employees, employeesWithoutMemberships } = useRouteLoaderData("userTable") as UserTableData;

  const userMembership = useMemo(() => userMemberships.find(user => user.id === userMembershipId)!, [userMembershipId, userMemberships]);
  const membershipEmployee = useMemo(() => employees.find(employee => employee.id === userMembership.employeeId) ?? null, [employees, userMembership]);
  ...
  const onSave = () => {submit({...userMembership, role: state.role, employeeId: employee?.id ?? null}, { method: "post", encType: "application/json" })};
  const displayName = getDisplayName(employee) ?? getDisplayName(userMembership);
    ...
    <ResponsiveDialog open>
      <ResponsiveDialogContent>
        <ResponsiveDialogTitle asChild>
          <Typography variant="h6">{t("Update membership of {{name}}", {name: displayName})}</Typography>
        </ResponsiveDialogTitle>
        <EmployeeSelection employee={employee} onChange={setEmployee} employees={employeeList} />
        ...
        <Stack direction="row" justifyContent="end" spacing={1}>
          ...
          <Tooltip title={t(disabledHelpText)}>
            <span>
              <Button variant="contained" color="primary" onClick={onSave} disabled={!!disabledHelpText}>{t("Save")}</Button>
            </span>
          </Tooltip>
        </Stack>
      </ResponsiveDialogContent>
...
function getDisabledText(state: RoleSelectionState, employee: Employee | null): string {
  if (state.role === null) {
